...
def group_contracts_by_strike(reference_wb):
    '''
    Given a workbook with many option contracs, sheet names that contain options with the same strike price are grouped together.
    Returns a dictionary with keys representing the strike and type of the option (call or put), and the value being a list of
    sheet names corresponding to each option for the given strike.
    '''
...
def create_sorted_sheet(new_workbook, reference_wb, new_sheet_title, reference_sheet_list, data_start_row, data_column, index_column):
    ...
    for (i,sheet_name) in enumerate(reference_sheet_list):  
        ...
        if i ==0:
            ...
            for (j,column_num) in enumerate(index_column+data_column):
                ...
                if j == 0:
                    copy_data(reference_sheet=data_sheet, main_sheet=new_sheet,index_start_row=data_start_row, index_end_row=max_row, reference_data_column=column_num, main_data_column=max_col)
                else:
                    copy_data(reference_sheet=data_sheet, main_sheet=new_sheet,index_start_row=data_start_row, index_end_row=max_row, reference_data_column=column_num, main_data_column=max_col+1)
